...
void setup() {
  Serial.begin(115200);
  Serial.println("Halo dari Julian!");

  dht.begin();

  WiFi.mode(WIFI_STA);
  WiFi.begin("Julian", "mango123");

  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    Serial.println("Proses Menghubungkan...");
  }
...
void loop() {
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    http.begin("http://192.168.127.76:5000/sensor/datasuhukelembapan");
